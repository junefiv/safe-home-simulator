...
**죽은 게 아니라, 그 칸의 VWorld 도로 API 응답을 기다리는 중일 가능성이 큽니다.**
...
- 도로/건물: `src/lib/server/map-cache.ts` → `.cache/map-cache/**` (없으면 **실시간 VWorld API**)
...
- [ ] Vercel 환경 변수 등록 (`VWORLD_*`, `SAFEMAP_*`, `ODCLOUD_*` 등)  
