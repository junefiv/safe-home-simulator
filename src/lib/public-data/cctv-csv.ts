...
const CSV_PATH = join(process.cwd(), ".cache", "CCTV정보.csv");
...
async function loadCctvCsv(): Promise<NormalizedFacility[]> {
  const bytes = await readFile(CSV_PATH);
  const lines = new TextDecoder("euc-kr").decode(bytes).split(/\r?\n/);
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    seen.add(key);
    facilities.push({
      id: `cctv-${at(row, "관리번호") || i}`,
      type: "cctv",
      lat,
      lng,
      name: `${at(row, "설치목적구분") || "방범"} CCTV`,
...
export function getCctvCsvFacilities(): Promise<NormalizedFacility[]> {
  if (!cachePromise) cachePromise = loadCctvCsv().catch(() => []);
  return cachePromise;
}
